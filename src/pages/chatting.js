...
const Chat = () => {
  ...
  useEffect(() => {
    localClient.init("8d4f054da71f427b93df3e27ca31bb54");
    localClient.login("test", "0068d4f054da71f427b93df3e27ca31bb54IAB4hZdWFs8F5RgLk74X0Yoabfs7TTwMZpiOKLJmBIdlKgx+f9gAAAAAEACmz2AjHyDtYAEA6AMfIO1g").then((e) => {
        localClient._logined = true;
        localClient.joinChannel("owake").then(() => {
            localClient.channels["owake"].joined = true;
        });
    });
  }, [])
  ...
  localClient.on('ChannelMessage', async ({ message, memberId }) => {
  ...
  const onSendMessage = () => {
    ...
    localClient.sendChannelMessage(message, "owake").then(() => {
        if(location[location.length - 1] == "right"){
            setLocation([...location, "left"])
        }else{
            setLocation([...location, "right"])
